<template>
  <div class="max-w-4xl mx-auto">
    ...
    <div v-else class="space-y-4">
      <div v-for="fav in favorites" :key="fav.id" class="bg-white rounded-lg shadow-sm p-4 hover:shadow-md transition-shadow">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center space-x-3 text-sm text-gray-500">
              <router-link :to="`/user/${fav.topic?.user_id}`" class="hover:text-blue-500">
                {{ fav.topic?.user?.nickname || fav.topic?.user?.username }}
              </router-link>
