...
type Report struct {
	...
	TargetType string     `gorm:"size:20;not null" json:"target_type"`   // 被举报对象类型：topic=话题, comment=评论, user=用户
	TargetID   uint       `gorm:"not null" json:"target_id"`             // 被举报对象ID
	Reason     string     `gorm:"size:50;not null" json:"reason"`        // 举报原因分类：spam/illegal/attack/rumor/other
